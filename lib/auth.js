...
module.exports = {
	...
	'getAuthToken': function(res, uri, cb, redir) {
		...
		if ('error' in uri.query) {
			...
			var onData = function(err, resp, c) {
				...
				try {
					z = JSON.parse(c);
					if (z.error === 'invalid_grant') {
						// meh, try again
						httpHeader(res, '/try_do_oauth');
					}
				} catch (e) {
					if (redir) {
				...
				var session = global.session.getSessionData(res.SESSID);
				if (redir) {
				...
				global.session.setSessionData(res.SESSID, session);
				if (typeof cb === 'function') {
					cb();
				}
			};
	...
	'refreshAuthToken': function(refreshToken, sessID, cb) {
		...
		var onData = function (err, res, c) {
			...
			var session = global.session.getSessionData(sessID);
			session.accessToken = z.access_token;
			session.accessTokenExpires = Date.now() + (1000 * 60 * 60);
			global.session.setSessionData(sessID, session);
			if (typeof cb === 'function') {
				cb();
			}
		};
