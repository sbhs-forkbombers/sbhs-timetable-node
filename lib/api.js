...
function getSHSAPI(path, token, params, post, cb) {
	'use strict';
	var req,
		b = function(err, resp, body) {
			console.log(require('util').inspect(resp));
			if (err || resp === undefined || resp === null) {
				cb({'error': err, 'status': 500});
			}
			if (resp.statusCode != 200) {
				console.log('error while getting ' + path + ': HTTP status ' + resp.statusCode);
				cb({'error': 'not ok', 'status': resp.statusCode});
				return;
			}
			var responded = false;
			var obj = {};
			try {
				...
				console.error('incomplete json response - ' + body, 'node err was', err);
				console.error(e.stack);
				cb({'error': 'not enough json', 'shs_response': body});
			}
		};
