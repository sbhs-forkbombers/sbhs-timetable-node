...
var apis = {
	...
	'daytimetable.json': function(opts, sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			...
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/timetable/daytimetable.json', sess.accessToken, params, false, cb);
		});
	},
	'timetable.json': function(opts, sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			...
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/timetable/timetable.json', sess.accessToken, params, false, cb);
		});
	},
	'dailynews.json': function(opts, sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			...
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/dailynews/list.json', sess.accessToken, params, false, cb);
		});
	},
	'participation.json': function(opts, sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/details/participation.json', sess.accessToken, {}, false, cb);
		});
	},
	'userinfo.json': function(opts, sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/details/userinfo.json', sess.accessToken, {}, false, cb);
		});
	},
	'barcodenews.json': function(opts, sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/barcodenews/list.json', sess.accessToken, {}, false, cb);
		});
	},
	'diarycalendar.json': function(opts, sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/diarycalendar/events.json', sess.accessToken, {}, false, cb);
		});
	},
	'calendar/days.json': function(opts,sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/calendar/days.json', sess.accessToken, opts, false, cb);
		});
	},
	'calendar/terms.json': function(opts,sessID, cb) {
		...
		auth.refreshTokenIfNeeded(sessID, function() {
			var sess = session.getSessionData(sessID);
			getSHSAPI('/api/calendar/terms.json', sess.accessToken, opts, false, cb);
		});
	},
		...
		apis['daytimetable.json'](opts, sessID, function(obj) {
			...
			if (Object.keys(obj.classVariations).length > 0) {
				...
				for (i in j){
					...
					variations[subjID].hasCover = (j[i].type !== 'nocover'); // TODO remove this in a few revisions
					variations[subjID].casual = j[i].casual;
					variations[subjID].casualDisplay = j[i].casualSurname;
					variations[subjID].cancelled = j[i].type === 'nocover';
			...
			var bells = {};
			for (i in obj.bells) {
				if (obj.bells[i].bell.match(/\d/)) {
//					bells[obj.bells[i].bell+'_begin'] = obj.bells[i];
//					bells[obj.bells[i].bell+'_end'] = obj.bells[Number(i)+1];
					var b = {};
					b.start = obj.bells[i].time;
					b.title = obj.bells[i].bellDisplay;
					var nxt = obj.bells[Number(i)+1];
					b.end = nxt.time;
					b.next = nxt.bellDisplay;
					bells[obj.bells[i].bell] = b;
				}
			}
			...
			var ary = prettified.today.split(' ');
			var dayNumber = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].indexOf(ary[0])+1;
			dayNumber += 5 * ['A','B','C'].indexOf(ary[1]);
			prettified.weekType = ary[1];
