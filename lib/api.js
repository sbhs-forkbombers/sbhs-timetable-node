...
String.prototype.endsWith = function(what) {
	return this.substr(this.length-what.length) == what;
}
...
var stressReliefURL = '/static/feelsgood.gif';
var stressReliefCache;

function updateStressReliefPhoto() {
	console.log('loading new stress relief photo');
	request('http://reddit.com/r/aww/.json', function(err, response, data) {
		stressReliefCache = false;
		if (err) {
			stressReliefURL = '/static/feelsgood.gif';
			console.log(':(', err);
			return;
		}
		try {
			...
			for (var i = 0; i < posts.length; i++) {
				if (posts[i].data.url.endsWith('jpg')) {
					stressReliefURL = posts[i].data.url;
					break;
				} else if (posts[i].data.url.startsWith('http://imgur.com/')) {
					stressReliefURL = posts[i].data.url + '.jpg';
					break;
				}
			}
...
updateStressReliefPhoto();
setInterval(updateStressReliefPhoto, 1000 * 60 * 60);
...
customAPIs['/api/picture.jpeg'] = function(req, res, session) {
	res.setHeader('Content-Type', 'image/jpeg');
	if (stressReliefURL.substr(0,4) == 'http') {
		var settings = {
			'url': stressReliefURL,
			'encoding': null
		}
		request(settings, function(err, response, data) {
			if (err) {
				res.status(302);
				res.setHeader('Location', '/static/feelsgood.gif');
				res.end();
				return;
			}
			res.end(data);
	...
	_getAPI('/api/timetable/daytimetable.json', session.accessToken, req.query, function(status, response) {
		...
		if (Object.keys(response.roomVariations).length > 0) {
			...
			for (i in temp) {
				...
				variations[subjID].roomFrom = temp[i].roomFrom;
				variations[subjID].roomTo 	= temp[i].roomTo;
			}
		}
...
customAPIs['/api/belltimes'] = function(req, res, session) {
	_getAPI('/api/timetable/bells.json', session.accessToken, req.query, function(status, obj) {
		...
		if (!req.accepts('application/json'))
			...
			res.type('json');
		if (status !== 200 || obj.status == "Error") {
			res.status(status);
			res.end(JSON.stringify(obj));
			return;
		}
...
module.exports.getStaticBelltimes = function() {
	var now = moment();
	if (now.day() == 0) {
		now.set({hour: 0, minute: 1}).add(1, 'days');
	} else if (now.day() == 6) {
		now.set({hour: 0, minute: 1}).add(2, 'days');
	} else if (now.isAfter({hour: 15, minute: 15})) {
		if (now.day() == 5) {
			now.set({hour: 0, minute: 1}).add(3, 'days');
		} else {
			now.set({hour: 0, minute: 1}).add(1, 'days');
		}
	...
	var bells = {
		bells: normalBells[now.format('ddd').toLowerCase()],
		status: 'CACHED',
		bellsAltered: true,
		bellsAlteredReason: 'Waiting for up to date belltimes…',
